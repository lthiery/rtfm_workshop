...
crate unsafe fn parse_and_validate(address: *const u8) -> Option<TbfHeader> {
    let version = *(address as *const u16);
    hprintln!("version {}", version).unwrap();
    let version1 = *(0x00020002 as *const u16);
    hprintln!("version {}", version1).unwrap();
    let version2 = *(0x00020004 as *const u16);
    hprintln!("version {}", version2).unwrap();
    let version3 = *(0x00020006 as *const u16);
    hprintln!("version {}", version3).unwrap();
    ...
    match version {
        ...
        // valid app.
        _ => {
            hprintln!("unrecognized version").unwrap();
